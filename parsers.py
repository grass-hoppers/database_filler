...
class Parser():
    ...
    def infinite_scroll(self, url='https://vc.ru/finance', ScrollNumber=10):
        prev = pd.DataFrame()
        SCROLL_PAUSE_TIME = 1
        ...
        for i in tqdm(range(1, ScrollNumber)):
            ...
            df = df.append(pd.DataFrame(
                temp, columns=['header', 'link', 'date', 'topic']))
    ...
    def help_button_scroll(self):
        ...
        for headers, links, time_txt, topics in zip(soup.find_all('a', {'class': 'link link_color '}),
                                                    soup.find_all(
                                                        'a', {'class': 'cover__link link cover__link_media'}),
                                                    soup.find_all(
                                                        'time', {'class': 'date'}),
            ...
            try:
                ...
                time_s = time.mktime(datetime.datetime.strptime(
                    f'{time_txt.attrs["datetime"]}', "%Y-%m-%d %H:%M").timetuple())
    ...
    def button_scroll(self, url='https://russian.rt.com/news', ScrollNumber=10):
    ...
    def parse_walking(self,
        ...
        for i in tqdm(range(pages_to_parse)):
            ...
            for article in soup.find_all(main_tag, {'class': main_class}):
                ref = article.find(
                    tag_of_link, {'class': class_of_link})['href']
